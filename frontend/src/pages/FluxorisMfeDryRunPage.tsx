...
type IntegrationModuleLike = IntegrationModule & { default?: IntegrationModule };
...
function inferSchemaForComponent(componentType: string): Record<string, unknown> {
  const key = String(componentType || '').toLowerCase();
  if (key === 'number' || key === 'decimal' || key === 'rating' || key === 'linear-scale' || key === 'slider') {
    return { type: 'number' };
  }
  if (key === 'checkbox') {
  ...
  if (key === 'single-choice-grid' || key === 'radio' || key === 'dropdown' || key === 'color-picker' || key === 'signature') {
    return { type: 'string' };
  }
  if (key === 'email') {
...
function deriveSchemaFromFormVersion(versionPayload: Record<string, unknown> | null): Record<string, unknown> | null {
  const version = (versionPayload?.version ?? versionPayload) as Record<string, unknown> | undefined;
  if (!version || typeof version !== 'object') return null;
  const pages = Array.isArray(version.pages) ? version.pages : [];

  const properties: Record<string, unknown> = {};
  const nonDataComponentTypes = new Set(['heading', 'section-divider', 'page-break']);

  for (const page of pages) {
    const pageObj = page as Record<string, unknown>;
    const components = Array.isArray(pageObj.components) ? pageObj.components : [];
    for (const comp of components) {
      const compObj = comp as Record<string, unknown>;
      const componentId = String(compObj.componentId || '').trim();
      const componentType = String(compObj.componentType || '').trim().toLowerCase();
...
): { schema: Record<string, unknown> | null; fieldMap: Record<string, string> } {
  const version = (versionPayload?.version ?? versionPayload) as Record<string, unknown> | undefined;
  if (!version || typeof version !== 'object') return { schema: null, fieldMap: {} };
  const pages = Array.isArray(version.pages) ? version.pages : [];
...
export default function FluxorisMfeDryRunPage() {
      ...
      .catch(() => {
        import('fluxorisPartnerMfe/PartnerIntegration')
          ...
          .catch((error: unknown) => {
            if (!mounted) return;
            const message =
              error instanceof Error ? error.message : 'Failed to load integration module.';
  ...
  useEffect(() => {
    ...
    async function loadSchemaFromForm() {
      ...
      try {
        setSchemaLoadError('');
        const payload = await api.get<Record<string, unknown>>(`/api/forms/${encodeURIComponent(trimmed)}/versions/latest`);
        if (cancelled) return;
        const { schema, fieldMap: derivedFieldMap } = deriveReadableSchemaAndFieldMap(payload);
        if (schema) {
  ...
  useEffect(() => {
    ...
    const remoteApi = getRemoteExport<
      { configureApiClient?: (options?: { baseUrl?: string; getToken?: () => string | null }) => void }
  ...
  const TemplateBuilderFlow = getRemoteExport<RemoteModule['TemplateBuilderFlow']>(
    remote,
    'TemplateBuilderFlow'
  );
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 p-6">
      ...
      <div className="rounded-md border p-4">
        ...
        <label className="mb-2 block text-sm font-medium">Fluxoris API Base URL</label>
        <input
          className="mb-3 w-full rounded border px-3 py-2 text-sm"
          value={fluxorisApiBase}
          onChange={(event) => setFluxorisApiBase(event.target.value)}
        />
        <label className="mb-2 block text-sm font-medium">Fluxoris App Base URL</label>
        <input
          className="mb-3 w-full rounded border px-3 py-2 text-sm"
      ...
      <div className="rounded-md border p-4">
        ...
        />
        {parsedSchema.error && (
          <p className="mt-2 text-sm text-red-600">Invalid schema JSON: {parsedSchema.error}</p>
        )}
      </div>
      ...
      {!loadError && !remote && (
        <div className="rounded-md border p-3 text-sm">Loading remote MFE...</div>
      )}

      {remote && (
        <>
          <div className="rounded-md border p-4">
            <h2 className="mb-3 text-base font-semibold">Template + Builder + Connect</h2>
            <label className="mb-2 block text-sm font-medium">Partner Status Webhook URL</label>
            <input
              className="mb-3 w-full rounded border px-3 py-2 text-sm"
            ...
            {!fluxorisToken && (
              <p className="text-sm text-amber-700">
                Get a Fluxoris token first to load template and node catalog data.
              </p>
            )}
            {connectedMessage && (
