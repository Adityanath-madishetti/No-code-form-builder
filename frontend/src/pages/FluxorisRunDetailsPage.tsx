...
export default function FluxorisRunDetailsPage() {
  const [ModuleComp, setModuleComp] = useState<PartnerRunDetailsComponent | null>(null);
  ...
  const fluxorisToken = (localStorage.getItem(FLUXORIS_PARTNER_TOKEN_KEY) || '').trim();
  ...
  async function resolveWorkflowAndRuns() {
    ...
    try {
      ...
      const connection = (await connectionRes.json()) as { workflow_id?: string };
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4 p-6">
      ...
      <div className="rounded-md border p-4">
        ...
        {workflowId && (
          <p className="mt-2 text-sm">Resolved Workflow ID: <code>{workflowId}</code></p>
        )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 rounded-md border p-4">
          ...
          {!loadError && !ModuleComp && (
            <div className="rounded-md border p-3 text-sm">Loading run details module...</div>
          )}

          {ModuleComp && selectedRunId && (
            <ModuleComp runId={selectedRunId} workflowId={workflowId || undefined} autoRefreshMs={0} />
          )}
        </div>
      </div>
    </div>
