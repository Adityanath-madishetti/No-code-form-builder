// backend/src/modules/ai/ai.prompt-builder.ts

import { ComponentRegistry } from './ai.registry.js';
...
function zodToPromptString(zodSchema: any) {
...
function buildComponentBlock(componentType: string, entry: any) {
...
export function generateBuilderPrompt(activeComponents: string[]) {
...
export function generateRouterCatalogue() {
  const lines = Object.entries(ComponentRegistry).map(
    ([type, entry]: any) => `- ${type}: ${entry.description}`,
  );
  return lines.join('\n');
}
